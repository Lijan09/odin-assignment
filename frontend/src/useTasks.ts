...
/** Shared shape for an in-place field edit: status or priority. */
export interface FieldState {
  saving: boolean
  error: string | null
}

const IDLE_ANALYSIS: AnalysisState = { loading: false, result: null, error: null }
...
export function useTasks(filter: Status | null) {
  ...
  const [statusStates, setStatusStates] = useState<Record<number, FieldState>>({})
  const [priorityStates, setPriorityStates] = useState<Record<number, FieldState>>({})
  ...
  const changeStatus = useCallback(
    async (id: number, status: Status) => {
      setStatusStates((prev) => ({ ...prev, [id]: { saving: true, error: null } }))
      try {
        const updated = await updateTaskStatus(id, status)
        setTasks((prev) => prev.map((task) => (task.id === id ? updated : task)))
        setStatusStates((prev) => ({ ...prev, [id]: { saving: false, error: null } }))
        void refreshCounts()
      } catch (error) {
        ...
        setStatusStates((prev) => ({ ...prev, [id]: { saving: false, error: message } }))
      }
    },
    [refreshCounts],
  )

  const changePriority = useCallback(async (id: number, priority: Priority) => {
    setPriorityStates((prev) => ({ ...prev, [id]: { saving: true, error: null } }))
    try {
      const updated = await updateTaskPriority(id, priority)
      setTasks((prev) => prev.map((task) => (task.id === id ? updated : task)))
      setPriorityStates((prev) => ({ ...prev, [id]: { saving: false, error: null } }))
    } catch (error) {
      // The stored task is left untouched, so the select falls back to its saved
      // value on the next render. Priority does not affect the tab counts.
      const message = error instanceof Error ? error.message : 'Could not save.'
  ...
  const analyse = useCallback(async (id: number) => {
    ...
    try {
      ...
      setAnalyses((prev) => ({ ...prev, [id]: { loading: false, result, error: null } }))
    } catch (error) {
      const message =
